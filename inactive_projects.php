...
if (isset($_POST['submit'])) {
  ...
  $plugin_search = $_POST['plugin_search'];
  $url_search = $_POST['url_search'];
  $industry_search = $_POST['industry_search'];
  $theme_search = $_POST['theme_search'];
  if (!empty($_POST['tag_search'])) :
    $tags_arr = explode(',', $_POST['tag_search']);
  else :
    $tags_arr = [];
  endif;
  if (!empty($_POST['plugin_search'])) :
    $plugin_arr = explode(',', $_POST['plugin_search']);
  else :
    $plugin_arr = [];
  endif;
  $query .= "
  ...
  if (!empty($plugin_arr) && count($plugin_arr) > 0) :
    foreach ($plugin_arr as $key => $val) :
      if ($val != '') :
        $query .= ' AND ';
        $query .= "FIND_IN_SET('" . trim($val) . "', plugins)";
      endif;
    endforeach;
  endif;
  if (!empty($url_search)) :
    if (!empty($industry_search) || !empty($theme_search) || (!empty($plugin_arr) && count($plugin_arr) > 0) || (!empty($tags_arr) && count($tags_arr) > 0)) :
  ...
  if (!empty($industry_search)) :
    if (!empty($url_search) || (!empty($tags_arr) && count($tags_arr) > 0) || !empty($theme_search) || (!empty($plugin_arr) && count($plugin_arr) > 0)) :
      $query .= " AND industry like '%" . $industry_search . "%'";
    else :
      $query .= "AND industry like '%" . $industry_search . "%'";
    endif;
  ...
  if (!empty($theme_search)) :
    if (!empty($url_search) || (!empty($tags_arr) && count($tags_arr) > 0) || !empty($industry_search) || (!empty($plugin_arr) && count($plugin_arr) > 0)) :
      $query .= " AND theme like '%" . $theme_search . "%'";
    else :
      $query .= "AND theme like '%" . $theme_search . "%'";
    endif;
...
#innertable td {
      ...
      <ul class="d-flex align-items-center">
        ...
        <li class="nav-item dropdown pe-3">
          ...
          <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
            <li>
              <a class="dropdown-item d-flex align-items-center" href="inactive_projects.php">
            ...
            <div class="card-body">
              ...
              <form class="row g-3" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                <div class="col-md-2">
                  ...
                  <div class="input-group">
                    <input type="text" name="theme_search" id="theme_search" placeholder="" autocomplete="off" class="form-control input-lg" value="<?php echo $theme_search; ?>" />
                  </div>
                  <br />
                </div>
                <div class="col-md-2">
                  ...
                  <div class="input-group">
                    <input type="text" name="plugin_search" id="plugin_search_data" placeholder="" autocomplete="off" class="form-control input-lg" value="<?php echo $plugin_search; ?>" />
                  </div>
                  <br />
                </div>
                <div class="col-md-2">
                  ...
                  <div class="input-group">
                    <input type="text" name="industry_search" id="industry_search" placeholder="" autocomplete="off" class="form-control input-lg" value="<?php echo $industry_search; ?>" />
                  </div>
                  <br />
                </div>
                          ...
                          <td colspan="6">
                            <table class="table" id="innertable">
                              <tr>
                                ...
                                <td width="30%">
                                  <div class="comment_content content_<?php echo $key; ?>" data-id="<?php echo $key; ?>" data-text="<?php echo $val['other_comments']; ?>">
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#plugin_search_data').tokenfield({
        autocomplete: {
          source: function(request, response) {

            jQuery.get('plugin_search.php', {
              query: request.term,
            }, function(data) {
              data = JSON.parse(data);
              response(data);
      ...
      $('#plugin_search_data').on('tokenfield:createtoken', function(event) {
        var existingTokens = $(this).tokenfield('getTokens');
        $.each(existingTokens, function(index, token) {
          if (token.value === event.attrs.value)
            event.preventDefault();
      ...
      $('#industry_search').tokenfield({
        autocomplete: {
          source: function(request, response) {
            $('#industry_search').parent().find('.token').remove();
            jQuery.get('industry_search.php', {
              query: request.term,
            }, function(data) {
              data = JSON.parse(data);
              response(data);
      ...
      $('#industry_search').on('tokenfield:createtoken', function(event) {
        $('#industry_search').parent().find('.token').remove();
        var existingTokens = $(this).tokenfield('getTokens');
        $.each(existingTokens, function(index, token) {
          if (token.value === event.attrs.value)
      ...
      $('#theme_search').tokenfield({
        autocomplete: {
          source: function(request, response) {
            $('#theme_search').parent().find('.token').remove();
            jQuery.get('theme_search.php', {
              query: request.term,
            }, function(data) {
              data = JSON.parse(data);
              response(data);
      ...
      $('#theme_search').on('tokenfield:createtoken', function(event) {
        $('#theme_search').parent().find('.token').remove();
        var existingTokens = $(this).tokenfield('getTokens');
        $.each(existingTokens, function(index, token) {
          if (token.value === event.attrs.value)
